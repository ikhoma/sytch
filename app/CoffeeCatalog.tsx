...
interface CoffeeCatalogProps {
  initialData: CoffeeItem[];
}

export default function CoffeeCatalog({ initialData }: CoffeeCatalogProps) {
  ...
  // Polling every 5 minutes (300000ms), fallback to server-rendered data
        ...
        <div className="flex flex-col items-start mb-6 md:mb-12 px-4 md:px-0">
          <h1 className="flex items-start min-h-[52px] md:min-h-[140px]">
            <span className="sr-only">
              {activeTab === 'Espresso'
                ? 'Espresso / Еспресо — каталог цін на свіжообсмажену каву Sytch'
                : 'Filter / Фільтр — каталог цін на свіжообсмажену каву Sytch'}
            </span>
            <img src="/espresso.svg" alt="" aria-hidden="true" className={`h-[52px] md:h-[119px] w-auto object-contain mt-[2px] md:mt-[5px] ${activeTab === 'Espresso' ? 'block' : 'hidden'}`} />
            <img src="/filter.svg" alt="" aria-hidden="true" className={`h-[42px] md:h-[97px] w-auto object-contain ${activeTab === 'Filter' ? 'block' : 'hidden'}`} />
          </h1>
        </div>
